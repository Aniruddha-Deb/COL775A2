...
def train(model, train_dl, val_dl, optimizer, scheduler=None, max_epochs=10, patience_lim=2, ckpt_freq=1, ckpt_prefix='runs/clevrer/videoclip/vclip'):
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch+1)%ckpt_freq == 0 and not DEBUG:
...
if __name__ == '__main__':
    ...
    if DEBUG:
        train_ds.json_data = train_ds.json_data[:8]
        #train_ds.json_data = [train_ds.json_data[322]]
        val_ds.json_data = val_ds.json_data[:1]
        n_epochs=2

    train_dl = DataLoader(train_ds, batch_size=1, collate_fn=dl_collate_fn, shuffle=True, num_workers=0, pin_memory=True)
    val_dl = DataLoader(val_ds, batch_size=1, collate_fn=dl_collate_fn, shuffle=False, num_workers=0, pin_memory=True)
